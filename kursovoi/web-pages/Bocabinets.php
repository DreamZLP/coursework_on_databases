<?php
session_start();
include '../functions/func_for_cabinets_table.php';
?>
<!DOCTYPE html>
<html lang="ru">
<head>
						...
						<table class="table shadow ">
							<thead class="thead-dark">
								<tr>
									...
									<th>Статус</th>
									<th>Площадь</th>
									<th>Номер заведующего</th>
									<th>Действие</th>
								</tr>
								<?php foreach ($result as $value) { ?>
								<tr>
									<td><?=$value['cabinet_number'] ?></td>
									<td><?=$value['cabinet_status'] ?></td>
									<td><?=$value['cabinet_square'] ?></td>
									<td><?=$value['manager_id'] ?></td>
									<td>
										<a href="?edit=<?=$value['cabinet_number'] ?>" class="btn btn-success btn-sm" data-toggle="modal" data-target="#editModal<?=$value['cabinet_number'] ?>"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
 											<path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z"/>
  											<path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z"/>
											</svg>
										</a> 
										<a href="?delete=<?=$value['cabinet_number'] ?>" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#deleteModal<?=$value['cabinet_number'] ?>"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash" viewBox="0 0 16 16">
										...
										if ($_SESSION['user']) {
											if ($_SESSION['user'][3] == 'system_user') {
												require 'modal_for_cabinets_table.php';
											}
										}
										?>
									</td>
					...
					<div class="modal-body">
					  <form action="" method="post">
						  <div class="form-group">
							  <input type="text" class="form-control" name="cabinet_number" value="" placeholder="Номер кабинета">
						  </div>
						  <div class="form-group">
							  <input type="text" class="form-control" name="cabinet_status" value="" placeholder="Статус кабинета">
						  </div>
						  <div class="form-group">
							  <input type="text" class="form-control" name="cabinet_square" value="" placeholder="Площадь кабинета">
						  </div>
						  <div class="form-group">
							  <input type="text" class="form-control" name="manager_id" value="" placeholder="Номер заведующего кабинетом">
						  </div>
					</div>
